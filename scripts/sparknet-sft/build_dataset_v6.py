...
Build SFT dataset for SparkNet-400M v6.

Key changes from v5:
- Replace UltraChat with smol-smoltalk (HuggingFaceTB/smol-smoltalk), which
  was synthesised for small models and produces responses that a 400M model
  can reasonably imitate.
- Data mix: 70 % smol-smoltalk / 30 % OASST-1.
- Relax assistant word cap to 200 (was 140) and max_assistant_tokens to 256
  (was 192) — the extra headroom gives the model room for multi-sentence
  answers without truncating mid-thought.
- Remove the user-side "programming request" filter — the assistant content
  filter already blocks code-heavy outputs, and the user filter was
  discarding too many legitimate conversations that happened to mention
  an API or regex.
- Increase max_messages to 8 (was 6).
"""
...
DEFAULT_OUTPUT_ROOT = "datasets/sft_chat_v6"
...
def main():
    ...
    filter_summary = {
        ...
        "note": "v6: programming-request user filter removed; word cap relaxed to 200",
    }
    ...
    for _ in range(args.num_shards):
        ...
        shard_metadata = {
            ...
            "note": (
                ...
                "Right-pad to block_size. v6: smol-smoltalk primary source, relaxed word cap."
            ),
        ...
        meta.setdefault("dataset", "sft_chat_v6")
