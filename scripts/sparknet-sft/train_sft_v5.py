...
SparkNet-400M SFT trainer for a narrower v5 chatbot regime.

v5 keeps the v4 non-packed format and eval cadence, but shifts emphasis toward:
- short explanations
- rewrites
- simple plans
- basic factual summaries
- clarification and uncertainty behavior

It also writes a structured checkpoint report on every eval step so behavioral
selection is visible alongside eval_loss.
"""
...
@dataclass
class RunConfig:
    run_name: str = "sparknet-400m-v1-instruct-v5"
    seed: int = 42

    model_path: str = "checkpoints/sparknet-400m-v1"
    tokenizer_path: str = "./tokenizer-v6"
    train_root: str = "datasets/sft_chat_v5"
    block_size: int = 1024
    ...
    target_tokens: int = 180_000_000

    eval_fraction: float = 0.02
    eval_steps: int = 100

    sample_prompts_path: str = "configs/sparknet-400m/sft_eval_prompts_v5.json"
    ...
    output_dir: str = "checkpoints/sparknet-400m-v1-instruct-v5"
...
def summarize_rows(ds: Dataset, block_size: int, n: int = 1024) -> Dict[str, int]:
    ...
    for idx in range(n):
        ...
        active = [label_idx for label_idx, label in enumerate(labels) if label != -100]
        if active:
...
def special_check(name: str, response: str) -> Dict[str, object]:
    ...
    if name == "ada_lovelace_basic":
        ...
        wrong = next(
            (
                token
                for token in ["french actress", "nobel", "dragon tattoo", "tolkien", "queen of hearts"]
                if token in lower
            ),
            None,
        )
...
def evaluate_response(prompt: Dict[str, object], response: str) -> Dict[str, object]:
    ...
    for needle in prompt.get("must_include_all", []) or []:
        needle = str(needle)
        add(f"has:{needle}", needle.lower() in response.lower(), "present" if needle.lower() in response.lower() else "missing")
    ...
    for needle in prompt.get("avoid_substrings", []) or []:
        needle = str(needle)
        add(f"avoid:{needle}", needle.lower() not in response.lower(), "ok" if needle.lower() not in response.lower() else "present")
...
class EvalReportCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, model=None, metrics=None, **kwargs):
        ...
        avg_score = round(sum(float(report["score"]) for report in prompt_reports) / max(1, len(prompt_reports)), 4)
