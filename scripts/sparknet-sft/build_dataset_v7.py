...
Build SFT dataset for SparkNet-400M v7 (ChatML format).

Key changes from v6:
- Switch from sparknet_chat_v1 (### System/User/Assistant: prefixes) to
  ChatML format (<|im_start|>/<|im_end|> special tokens).
- Add <|im_start|> (id=32000) and <|im_end|> (id=32001) as additional
  special tokens so both HF and llama.cpp treat them as atomic units.
- This aligns role-boundary tokens between HF training and GGUF inference
  (special tokens are always recognized identically in both stacks).
- Dataset mix and filtering rules unchanged from v6.
"""
...
DEFAULT_OUTPUT_ROOT = "datasets/sft_chat_v7"
...
CHAT_TEMPLATE_VERSION = "chatml_v1"
IM_START = "<|im_start|>"
IM_END = "<|im_end|>"

DEFAULT_MAX_ASSISTANT_TOKENS = 256
DEFAULT_MAX_MESSAGES = 8

DATA_SOURCES = [
    ("HuggingFaceTB/smol-smoltalk", 0.70),
    ("OpenAssistant/oasst1", 0.30),
]
...
_WORKER_PAD = None
_WORKER_CFG = None
_WORKER_ROLE_HEADER_IDS = None   # role -> tokenized "<|im_start|>{role}\n"
_WORKER_CLOSER_IDS = None        # tokenized "<|im_end|>\n"
...
def load_sparknet_tokenizer(tokenizer_path: str):
    ...
    # Add ChatML boundary tokens as special tokens so both HF and llama.cpp
    # treat them as single atomic IDs (32000, 32001).
    new_tokens = [t for t in [IM_START, IM_END] if t not in tok.get_vocab()]
    if new_tokens:
...
def _init_worker(tokenizer_dir: str, block_size: int, max_assistant_tokens: int):
    global _WORKER_TOK, _WORKER_EOS, _WORKER_PAD, _WORKER_CFG
    global _WORKER_ROLE_HEADER_IDS, _WORKER_CLOSER_IDS
    ...
    # Pre-tokenize the ChatML structural strings once per worker.
    # add_special_tokens=False prevents automatic BOS/EOS injection;
    # <|im_start|>/<|im_end|> are still recognized as their own token IDs.
    _WORKER_ROLE_HEADER_IDS = {
        role: tok(f"{IM_START}{role}\n", add_special_tokens=False)["input_ids"]
        for role in ("system", "user", "assistant")
    }
    _WORKER_CLOSER_IDS = tok(f"{IM_END}\n", add_special_tokens=False)["input_ids"]
...
def _tokenize_messages(messages: List[dict]) -> Tuple[List[int], List[int], int]:
    ...
    #   input_ids : <|im_start|> role \n  content  <|im_end|> \n
    #   labels    :    -100      -100  -100  content  <|im_end|> \n   (assistant only)
    #              :    -100      -100  -100   -100     -100      -100  (system / user)
    for msg in messages:
        role = msg["role"]
        header_ids = _WORKER_ROLE_HEADER_IDS.get(role)
        ...
        seg_ids = header_ids + content_ids + _WORKER_CLOSER_IDS
        input_ids.extend(seg_ids)

        if role == "assistant":
            labels.extend([-100] * len(header_ids))
            labels.extend(content_ids + _WORKER_CLOSER_IDS)
            supervised += len(content_ids) + len(_WORKER_CLOSER_IDS)
        else:
...
def smoltalk_stream(max_messages: int) -> Iterator[List[dict]]:
    ds = _load_split("HuggingFaceTB/smol-smoltalk", ["train"], streaming=True)
    for row in ds:
...
def oasst_stream(max_messages: int, only_en: bool = True) -> Iterator[List[dict]]:
    ...
    for root in roots:
        ...
        while current:
            ...
            candidates = [
                c
                for c in children.get(current["message_id"], [])
...
def _init_source_iterators(max_messages: int):
    streams = {
        "HuggingFaceTB/smol-smoltalk": smoltalk_stream(max_messages=max_messages),
...
def _next_source_item(iters, names, weights, rng, max_messages: int):
    while True:
        ...
        try:
            ...
            if source == "HuggingFaceTB/smol-smoltalk":
                iters[source] = iter(smoltalk_stream(max_messages=max_messages))
            else:
...
def main():
    ...
    filter_summary = {
        ...
        "assistant_word_cap": 200,
        "max_assistant_tokens": MAX_ASSISTANT_TOKENS,
        "max_messages": MAX_MESSAGES,
        "note": "v7: ChatML format; same data mix and filters as v6",
    }
    ...
    for _ in range(args.num_shards):
        ...
        shard_metadata = {
            ...
            "note": (
                ...
                "Role header masked; supervise assistant content + <|im_end|> + newline + eos. "
                "Right-pad to block_size. v7: ChatML for GGUF-compatible role boundaries."
            ),
        ...
        meta.setdefault("dataset", "sft_chat_v7")
        ...
        meta.setdefault("im_start", IM_START)
        meta.setdefault("im_end", IM_END)
