...
SparkNet-400M SFT trainer, v7 (ChatML format).

Changes from v6:
- Uses sft_chat_v7 dataset (ChatML role boundaries).
- Adds <|im_start|> (id=32000) and <|im_end|> (id=32001) as special tokens
  and resizes the embedding table from 32000 → 32002 after model load.
- build_prompt / stop_at_next_role_header updated for ChatML.
- evaluate_response no_role_leak check updated to detect <|im_start|> leakage
  instead of ### prefix leakage.
"""
...
class RunConfig:
    ...
    output_dir: str = "checkpoints/sparknet-400m-v2-instruct-v2"
...
def stop_at_next_role_header(text: str) -> str:
    # The model ends its turn with <|im_end|>; strip it and anything after.
    # Also guard against a stray new turn starting with <|im_start|>.
    cut = None
...
def evaluate_response(prompt: Dict[str, object], response: str) -> Dict[str, object]:
    ...
    role_leak = IM_START in response or IM_END in response
    add("no_role_leak", not role_leak,
        "ok" if not role_leak else "ChatML delimiter leaked into response")
...
def main():
    ...
    if emb_vocab > tok_vocab:
        die(
            ...
            "Unexpected state — check model and tokenizer."
        )
    if emb_vocab < tok_vocab:
        # v7: tokenizer gained <|im_start|> and <|im_end|> (32000, 32001).
        # Resize the embedding table; new rows are initialized to the mean
        # embedding by HF's default, which is a reasonable starting point.
        model.resize_token_embeddings(tok_vocab)
        print(f"[Harmony] Resized embeddings: {emb_vocab} → {tok_vocab}")
    ...
    print(
        f"[Harmony] tok_vocab={tok_vocab} emb_vocab={model.get_input_embeddings().weight.shape[0]} "
    ...
    # Embed the ChatML Jinja2 template in tokenizer_config.json so that
    # convert_hf_to_gguf.py can bake it into the GGUF and llama-server will
    # apply it automatically without needing --chat-template-file.
    tok.chat_template = (
        "{% for message in messages %}"
