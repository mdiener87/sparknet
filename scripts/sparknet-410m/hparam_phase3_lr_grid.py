...
OUTPUT (per run):
  logs/hparam-410m-phase3-grid-lr{N}/  — TensorBoard + loss curve JSON
  logs/hparam-410m-phase3-grid-summary.json  — cross-run comparison + recommendation
"""
import argparse
import copy
import json
...
DEFAULT_SCREEN_LRS = [5.5e-4, 9e-4, 1.2e-3, 1.83e-3]
...
def run_one(
    ...
    run_name = f"hparam-410m-phase3-grid-lr{lr_tag}"
...
def print_summary(results: list):
    ...
    best_score = sort_key(ranked[0])
    stable_close = [
        r for r in ranked
        if not r.get("has_spikes") and sort_key(r) <= best_score + 0.02
    ]
    winner = min(stable_close, key=lambda r: r["lr"]) if stable_close else ranked[0]
...
def main():
    ...
    elif args.ceiling_lr:
        c = args.ceiling_lr
        grid_lrs = sorted([c / 10.0, c / 3.0, c / 1.5])
    else:
        grid_lrs = DEFAULT_SCREEN_LRS
    ...
    results = []
    for lr in grid_lrs:
        result = run_one(lr, args.grad_accum, args.target_tokens, train_ds, eval_ds, init_state_dict, tok)
        results.append(result)

    winner = print_summary(results)

    # Save summary
    summary_path = REPO_ROOT / "logs" / "hparam-410m-phase3-grid-summary.json"
