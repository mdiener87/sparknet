"""
Orchestrator: runs Phase 2 → 3 → 4 in sequence, feeding each phase's
output into the next automatically.

  Phase 2 → finds LR ceiling → writes lr_range_analysis.json
  Phase 3 → runs 3-point grid at that ceiling → writes grid-summary.json
  Phase 4 → tests warmup sensitivity at Phase 3 winner LR (optional)

USAGE:
  # Full run (recommended — run in tmux or screen so it survives disconnect)
  python3 hparam_run_all.py --grad-accum 32

  # Skip warmup phase (saves ~11 hrs at 7K tok/s)
  python3 hparam_run_all.py --grad-accum 32 --skip-phase4

  # Resume from Phase 3 if Phase 2 already finished
  python3 hparam_run_all.py --grad-accum 32 --start-from 3

  # Resume from Phase 4 with an explicit LR
  python3 hparam_run_all.py --grad-accum 32 --start-from 4 --winner-lr 3e-4
"""
import argparse
import json
import subprocess
...
def read_ceiling_lr() -> float:
    path = REPO_ROOT / "logs" / "hparam-phase2-lr-range" / "lr_range_analysis.json"
    if not path.exists():
...
def read_winner_lr() -> float:
    path = REPO_ROOT / "logs" / "hparam-phase3-grid-summary.json"
    if not path.exists():
...
def main():
    ...
    parser.add_argument(
        "--train-root", default="datasets/sparknet-v6-pretrain",
    )
    parser.add_argument(
        "--tokenizer-path", default="./tokenizer-v6",
    )
    parser.add_argument(
        "--start-from", type=int, choices=[2, 3, 4], default=2,
        help="Resume from this phase (2=default, assumes prior phases already ran)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Skip the warmup sensitivity phase (saves ~11 hrs at 7K tok/s)",
    )
    parser.add_argument(
    ...
    if args.start_from <= 3:
        ...
        run_phase(
            [python, str(SCRIPT_DIR / "hparam_phase3_lr_grid.py"),
             "--ceiling-lr", str(ceiling_lr)] + common,
            "Phase 3 — 3-Point LR Grid",
        )
    ...
    warmup_rec = None
    phase4_summary = REPO_ROOT / "logs" / "hparam-phase4-warmup-summary.json"
