...
OUTPUT:
  logs/hparam-phase4-warmup-{ratio}/  — TensorBoard + loss curve JSON
  logs/hparam-phase4-warmup-summary.json  — comparison + recommendation
"""
import argparse
import copy
import json
...
def run_one(
    ...
    run_name = f"hparam-phase4-warmup-{ratio_tag}"
...
def main():
    ...
    for wr in sorted(args.warmup_candidates):
        result = run_one(args.lr, wr, args.grad_accum, train_ds, eval_ds, init_state_dict, tok)
        results.append(result)

    print_summary(results, args.lr)

    summary_path = REPO_ROOT / "logs" / "hparam-phase4-warmup-summary.json"
