...
# Run Configuration
RUN_NAME = "sparknet-70m-v4"

# --------------------------------------------------------------------------------------
# Load Config
# --------------------------------------------------------------------------------------
with open("configs/datasets_v4.json") as f:
    cfg = json.load(f)
...
target_tokens = int(cfg.get("target_tokens", 2_000_000_000))  # training budget
...
mixture_stream = mixture_stream.shuffle(seed=seed, buffer_size=10_000)
...
peek = next(iter(gen_train()))
assert len(peek["input_ids"]) == block_size, "Packing generator misaligned"
...
print(f"Using {torch.cuda.device_count()} GPU(s) | batch={per_device_train_batch_size} | grad_accum={gradient_accumulation_steps}")
...
if __name__ == "__main__":
    ...
    metadata = {
        ...
        "notes": "V4 | Improved training run with 1bil unique training tokens, run twice for 2bil total tokens."
    }
