#!/usr/bin/env python3
"""
Pack a pretraining shard from the sparknet-125m corpus.
...
Run once per shard. For 2.5B training tokens at 500M tokens/shard, run 5×.
For the eval shard, use datasets_v1_eval.json and run once.

Shard disjointness (fix for the 410m-v1 duplication bug)
--------------------------------------------------------
The 410m-v1 build invoked this script once per shard, and each invocation
opened the HuggingFace sources with streaming=True, which ALWAYS restarts at
document 0. The per-shard rng only reshuffled how the sources were interleaved,
not which documents each source yielded — so every shard repacked the first
~500M tokens of each source. The result was ~21x duplication (~611M unique
tokens repeated ~17 epochs) that went unnoticed for the entire 16-day run.

The fix: each document is deterministically assigned to exactly one of
`num_shards` buckets by hashing its content (the same mechanism the train/eval
holdout already uses). Shard k keeps only the documents whose bucket == k, so
the train shards are disjoint by construction even though every invocation still
streams each source from position 0. See `_accept_for_shard`. The eval
partition is a single shard and is not bucketed.

Usage:
  # Build one training shard (run num_shards times; the next free index is used)
  python build_dataset.py --config configs/sparknet-125m/datasets_v1.json

  # Build the eval shard (run once before training)
  python build_dataset.py --config configs/sparknet-125m/datasets_v1_eval.json
"""
...
DEFAULT_CONFIG_PATH = "configs/sparknet-125m/datasets_v1.json"
...
DEFAULT_OUTPUT_ROOT = "datasets/sparknet-125m-v1-pretrain"
...
DEFAULT_HOLDOUT_SALT = "sparknet-125m-v1"
...
def load_config(path: str) -> Dict:
    ...
    cfg.setdefault("num_shards", DEFAULT_NUM_SHARDS)
    if cfg["partition"] == "eval":
        # The eval partition is a single shard; bucketing would needlessly
        # discard most of the (already tiny) holdout.
        cfg["num_shards"] = 1
    cfg["num_shards"] = int(cfg["num_shards"])
    if cfg["num_shards"] < 1:
        raise ValueError("Config `num_shards` must be >= 1.")
...
def _shard_bucket(text: str, source: Dict, num_shards: int) -> int:
    """Deterministically map a document to one of `num_shards` buckets.

    Uses a salt distinct from the partition/chatml hashes so bucket assignment
    is statistically independent of the train/eval split. The clamp guards the
    (measure-zero) case where _hash_ratio returns exactly 1.0.
    """
    bucket = int(_hash_ratio(source_name(source), text, str(source["holdout_salt"]), "shard") * num_shards)
    return min(bucket, num_shards - 1)


def _accept_for_shard(text: str, source: Dict) -> bool:
    """True iff this document belongs to the shard currently being built.

    This is the fix for the 410m-v1 duplication bug: because every build
    invocation re-streams each source from document 0, disjointness has to come
    from the content hash, not from stream position. With num_shards == 1 (e.g.
    the eval partition) every document is accepted.
    """
    num_shards = int(source.get("_num_shards", 1))
    if num_shards <= 1:
        return True
    return _shard_bucket(text, source, num_shards) == int(source["_shard_id"])
                ...
                if isinstance(row, dict):
                    ...
                    for text in extract_texts(row, source):
                        ...
                        if not _accept_for_partition(stripped, source):
                            ...
                            continue
                        if not _accept_for_shard(stripped, source):
                            _bump(source, "documents_other_shard")
                            continue
...
def next_shard_dir(output_root: str, num_shards: int) -> Path:
    ...
    if len(existing) >= num_shards:
        raise RuntimeError(
            f"All {num_shards} shards already exist under {root}. Bucket assignment "
            f"is fixed by num_shards; building a {len(existing)}th shard would "
            f"duplicate an existing bucket. Increase num_shards or clean up first."
        )
...
def shard_generator(cfg: Dict, shard_id: int):
    ...
    for source in sources:
        ...
        source["_shard_id"] = shard_id
        source["_num_shards"] = num_shards
...
def main():
    ...
    shard_dir = next_shard_dir(str(cfg["output_root"]), int(cfg["num_shards"]))
    shard_id = int(shard_dir.name.split("-")[-1])

    print(f"Config:        {args.config}")
    print(f"Output shard:  {shard_dir}  (bucket {shard_id} of {int(cfg['num_shards'])})")
